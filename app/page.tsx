...
// =============================================================================
// КОНСТАНТЫ (дублирую для надёжности)
// =============================================================================
  ...
  return (
    <>
      <button
        ...
        className="lg:hidden fixed top-4 right-4 z-50 w-12 h-12 bg-white rounded-full shadow-lg flex items-center justify-center"
      ...
      <div className={`
        lg:hidden fixed top-0 right-0 h-full w-72 bg-white z-50 shadow-xl
      ...
      `}>
        <div className="p-6">
          ...
          <nav className="space-y-4">
            {navItems.map((item) => (
              <a key={item.href} href={item.href} onClick={() => setIsOpen(false)} className="block py-3 text-lg text-warm-700">
                {item.label}
              </a>
            ))}
          </nav>
          ...
          <div className="mt-8 pt-8 border-t border-warm-200">
            <a href={`tel:${CLINIC_PHONE}`} className="flex items-center gap-3 text-warm-700 font-medium">
...
function DesktopNav() {
  ...
  return (
    <nav className="hidden lg:flex items-center gap-8 absolute top-6 left-8">
      {navItems.map((item) => (
        <a key={item.href} href={item.href} className="text-warm-700 hover:text-coral-500 font-medium transition-colors">
          {item.label}
        </a>
      ))}
    </nav>
...
function CallButton({ className = "" }: { className?: string }) {
  return (
    <a href={`tel:${CLINIC_PHONE}`} className={`btn-primary flex items-center gap-3 ${className}`}>
...
function DirectionsButton({ className = "" }: { className?: string }) {
  ...
  return (
    <button onClick={handleOpenMaps} disabled={loading} className={`btn-secondary flex items-center gap-2 ${className}`}>
      <RouteIcon className="w-5 h-5" />
      {loading ? "Загрузка..." : "Маршрут"}
    </button>
  );
}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-12 items-center">
          <div className="text-center lg:text-left animate-fade-in order-2 lg:order-1">
            ...
            <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-warm-800 mb-4 md:mb-6">
              {DOCTOR_NAME}
            </h1>
    ...
    <section id="services" className="py-16 md:py-22 bg-white">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-10 md:mb-16">
          <h2 className="font-serif text-2xl md:text-4xl text-warm-800 mb-3 md:mb-4">Моя специализация</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {services.map((service) => (
            <div key={service.title} className="card text-center p-4 md:p-6">
              <div className="w-12 h-12 md:w-14 md:h-14 mx-auto mb-3 md:mb-4 rounded-xl bg-coral-100 flex items-center justify-center">
    ...
    <section id="contacts" className="py-16 md:py-22 bg-gradient-to-b from-white to-cream-100">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="font-serif text-2xl md:text-4xl text-warm-800 mb-4 md:mb-6">Запишитесь на приём</h2>
          <p className="text-warm-600 mb-8 md:mb-10 text-sm md:text-base">Приходите по адресу: {CLINIC_ADDRESS}</p>
...
function Footer() {
  return (
    <footer className="py-6 md:py-8 bg-warm-800 text-white/70 text-center">
      <p className="text-xs md:text-sm px-4">© {new Date().getFullYear()} {DOCTOR_NAME}. Все права защищены.</p>
    </footer>
  );
}

// =============================================================================
// PLACEHOLDERS ДЛЯ КОМПОНЕНТОВ
// =============================================================================

function PatientEducationHubPlaceholder() {
  return (
    <section className="max-w-3xl mx-auto p-6 mb-12 bg-white rounded-2xl shadow-lg font-sans">
      <h2 className="text-3xl font-bold text-teal-600 mb-8 text-center">Образовательный центр для пациентов</h2>
      <div className="grid gap-4 md:gap-6">
        {[
          { title: "Профилактика кариеса", short: "Как защитить зубы от разрушения." },
          { title: "Этапы протезирования зубов", short: "Что происходит на каждом этапе лечения." },
        ].map((item) => (
          <div key={item.title} className="p-4 rounded-xl bg-cream-100 border border-warm-200">
            <h3 className="text-lg font-semibold text-warm-800 mb-2">{item.title}</h3>
            <p className="text-warm-600 text-sm">{item.short}</p>
          </div>
        ))}
      </div>
    </section>
...
function SymptomCalculatorPlaceholder() {
  return (
    <section className="max-w-xl mx-auto p-6 bg-cream-100 rounded-2xl shadow-lg my-12">
      <h2 className="text-2xl font-semibold text-teal-600 mb-6">Интерактивный калькулятор симптомов</h2>
      <p className="text-warm-600">Выберите симптомы, чтобы получить рекомендации.</p>
    </section>
  );
}
  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="lg:pl-64">
        <HeroSection />
        <ServicesSection />
        <PatientEducationHubPlaceholder />
        <SymptomCalculatorPlaceholder />
        <ContactsSection />
        <Footer />
      </div>
    </main>
